...
from zipline.finance.transaction import create_transaction, create_aqm_transaction
...
# CHG [joseph]: Customized slippage model
class AQMVolumeShareSlippage(SlippageModel):
    """
    Model slippage as a function of the volume of contracts traded.
    """
    def __init__(self, volume_limit=DEFAULT_EQUITY_VOLUME_SLIPPAGE_BAR_LIMIT,
        ...
        super(AQMVolumeShareSlippage, self).__init__()
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def process_order(self, data, order):
        ...
        # Get commission information from minute data; if fail, try daily data instead
        try:
            commission = data.history(order.asset, "commission", 10, '1m').iloc[-1]
        except:
            commission = data.history(order.asset, "commission", 10, '1d').iloc[-1]
        try:
            commission_type = data.history(order.asset, "commission_type", 10, '1m').iloc[-1]
        except:
            commission_type = data.history(order.asset, "commission_type", 10, '1d').iloc[-1]
    ...
    def simulate(self, data, asset, orders_for_asset):
        ...
        for order in orders_for_asset:
            ...
            try:
                execution_price, execution_volume, commission, commission_type = \
                    self.process_order(data, order)

                if execution_price is not None:
                    txn = create_aqm_transaction(
                        order,
                        data.current_dt,
                        execution_price,
                        execution_volume,
