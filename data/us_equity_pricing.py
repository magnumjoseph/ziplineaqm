...
# CHG [joseph]: add ``commission`` as additional information in OHLC
OHLCMORE = frozenset(['open', 'high', 'low', 'close', 'commission'])
OHLC = frozenset(['open', 'high', 'low', 'close'])

# CHG [joseph]: add ``commission`` and ``commission_type`` as additional columns
AQM_EQUITY_PRICING_BCOLZ_COLUMNS = (
    'open', 'high', 'low', 'close', 'volume', 'commission', 'commission_type', 'day', 'id'
)
...
class BcolzDailyBarWriter(object):
    ...
    def _write_internal(self, iterator, assets):
        ...
        # }
        # CHG [joseph]: add ``commission`` and ``commission_typ`` to the columns
        columns = {
            k: carray(array([], dtype=uint32))
            for k in AQM_EQUITY_PRICING_BCOLZ_COLUMNS
        }
        ...
        full_table = ctable(
            columns=[
                columns[colname]
                for colname in AQM_EQUITY_PRICING_BCOLZ_COLUMNS
            ],
            names=AQM_EQUITY_PRICING_BCOLZ_COLUMNS,
            rootdir=self._filename,
            mode='w',
        )
    ...
    def to_ctable(self, raw_data, invalid_data_behavior):
        ...
        winsorise_uint32(raw_data, invalid_data_behavior, 'volume', *OHLCMORE)
        processed = (raw_data[list(OHLCMORE)] * 1000).astype('uint32')
        dates = raw_data.index.values.astype('datetime64[s]')
        check_uint32_safe(dates.max().view(np.int64), 'day')
        processed['day'] = dates.astype('uint32')
        processed['volume'] = raw_data.volume.astype('uint32')
        processed['commission_type'] = raw_data['commission_type'].astype('uint32')
