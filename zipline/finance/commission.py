...
# CHG [joseph]: Customized commission model
# DEFAULT_PER_DOLLAR_LOT = 12
class PerTransaction(CommissionModel):
    """
    Calculates a commission for a transaction based on a per dollar cost OR per lot cost.
    ...
    cost : float
        The flat amount of commissions paid per dollar of equities traded.
    """
    # def __init__(self, cost_per_dollar=DEFAULT_PER_DOLLAR_COST, cost_per_lot=DEFAULT_PER_DOLLAR_LOT):
    #     """
    #     Cost parameter is the cost of a trade per-dollar. 0.0015
    #     on $1 million means $1,500 commission (=1M * 0.0015)
    #     """
    #     self.cost_per_dollar = float(cost_per_dollar)
    #     self.cost_per_lot = float(cost_per_lot)
    
    def __init__(self):
        """
        No need to specify commission rate in instantiation since this value is given in current data
        """
        pass
