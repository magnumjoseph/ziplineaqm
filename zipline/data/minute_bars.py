...
def convert_cols(cols, scale_factor, sid, invalid_data_behavior):
    ...
    # CHG [joseph]: add ``commission`` and ``commission_type``
    scaled_commission = np.nan_to_num(cols['commission']) * scale_factor
    ...
    # CHG [joseph]: add ``commission`` and ``commission_type``
    commission = scaled_commission.astype(np.uint32)
    commission_type = cols['commission_type'].astype(np.uint32)
    ...
    # CHG [joseph]: add ``commission`` and ``commission_type``
    commission[exclude_mask] = 0
    commission_type[exclude_mask] = 0

    # CHG [joseph]: add ``commission`` and ``commission_type``
    return opens, highs, lows, closes, volumes, commission, commission_type
...
class BcolzMinuteBarWriter(object):
    ...
    # CHG [joseph]: add ``commission`` and ``commission_type``
    COL_NAMES = ('open', 'high', 'low', 'close', 'volume', 'commission', 'commission_type')
    ...
    def _init_ctable(self, path):
        ...
        table = ctable(
            ...
            columns=[
                ...
                initial_array,
                initial_array,
                # CHG [joseph]: add two more columns ``commission`` and ``commission_type``
                initial_array,
                initial_array,
            ],
            names=[
    ...
    def write_sid(self, sid, df, invalid_data_behavior='warn'):
        ...
        cols = {
            ...
            'close': df.close.values,
            'volume': df.volume.values,
            'commission': df.commission.values,
            'commission_type': df.commission_type.values,
        }
        dts = df.index.values
    ...
    def _write_cols(self, sid, dts, cols, invalid_data_behavior):
        ...
        # CHG [joseph]: add ``commission`` and ``commission_type``
        commission_col = np.zeros(minutes_count, dtype=np.uint32)
        commission_type_col = np.zeros(minutes_count, dtype=np.uint32)
        ...
        (
            ...
            vol_col[dt_ixs],
            # CHG [joseph]: add ``commission`` and ``commission_type``
            commission_col[dt_ixs],
            commission_type_col[dt_ixs],
...
class BcolzMinuteBarReader(MinuteBarReader):
    ...
    #     'volume': 1550,
    # }
    # CHG [joseph]: add two more fields ``commission`` and ``commission_type``
    FIELDS = ('open', 'high', 'low', 'close', 'volume','commission', 'commission_type')
    DEFAULT_MINUTELY_SID_CACHE_SIZES = {
        'close': 3000,
        'open': 1550,
        'high': 1550,
    ...
    def load_raw_arrays(self, fields, start_dt, end_dt, sids):
        ...
        for field in fields:
            # if field != 'volume':
            # CHG [joseph]: we treat ``commission_type`` same way as ``volume``
            if field not in ('volume', 'commission_type'):
